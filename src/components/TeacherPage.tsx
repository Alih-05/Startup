...
export const TeacherPage: React.FC<TeacherPageProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-site-bg p-6 transition-colors duration-500">
            ...
            <div className="flex justify-between items-start">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="px-2 py-1 bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-wider rounded-md">
                    {t('mentorBadge')}
                  </span>
                </div>
                <p className="text-primary font-medium">{t('teacherBio')}</p>
              </div>
            </div>
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="md:col-span-2 space-y-8">
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(t('teacherSkills') as any[]).map((skill, i) => (
                      <span key={`${skill}-${i}`} className="px-4 py-2 bg-gray-50 border border-gray-100 rounded-xl text-sm font-medium text-gray-700">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              <div className="space-y-6">
                <div className="p-6 bg-primary/5 rounded-2xl border border-primary/10">
                  <h3 className="font-bold text-primary mb-2">{t('teacherStatsTitle')}</h3>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-primary/80">{t('teacherStatsProjects')}</span>
                      <span className="font-bold text-indigo-900">25+</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-primary/80">{t('teacherStatsStudents')}</span>
                      <span className="font-bold text-indigo-900">100+</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-primary/80">{t('teacherStatsExperience')}</span>
